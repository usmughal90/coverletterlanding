"use client";

import Image from "next/image";
import Link from "next/link";
import DownloadButton from "@/app/components/shared/DownloadButton";

interface HeroSectionProps {
  title?: React.ReactNode;
  subtitle?: string;
}

export default function HeroSection({ title, subtitle }: HeroSectionProps) {
  return (
    <section
      id="home"
      className="relative w-full pt-30 pb-20 md:pt-24 md:pb-16 lg:pt-28 lg:pb-20 min-h-screen flex items-center"
    >
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <Image
          src="/images/bg.png"
          alt="Background"
          fill
          priority
          className="object-cover"
        />
      </div>

      {/* Main Container */}
      <div className="relative z-30 mx-auto w-full max-w-screen-2xl px-6 md:px-12 lg:px-20">
        <div className="flex flex-col items-center justify-center gap-12 lg:flex-row lg:items-center">
          {/* Left Side: Content */}
          <div className="w-full text-center  lg:text-left  order-2 lg:order-1 ">
            <h1 className="text-balance text-4xl font-bold leading-[1.1] tracking-tight text-white sm:text-6xl lg:5xl xl:text-6xl 2xl:text-7xl">
              {title || <>Cover Letter AI & Resume Maker</>}
            </h1>

            <p className="mt-4 mx-auto max-w-lg text-base leading-relaxed text-white/90 md:text-lg lg:mx-0">
              {subtitle ||
                "The No.1 App to create professional CV and Job Cover Letters on App Store."}
            </p>

            {/* Buttons */}
            <div className="mt-8 flex flex-col md:flex-row items-stretch lg:items-start gap-4 w-full sm:w-auto justify-center lg:justify-start">
              <DownloadButton className="w-full sm:w-[220px] md:w-[240px] lg:w-[260px] h-[64px] flex items-center justify-center" />

              <Link
                href="#about"
                className="w-full sm:w-[220px] md:w-[240px] lg:w-[260px] 
                h-[64px] 
                flex items-center justify-center
                bg-[#37dda4]  
                text-[#0F172A] text-sm sm:text-lg 
                rounded-full shadow-2xl 
                transform transition hover:scale-105 active:scale-95 text-center"
              >
                LEARN MORE
              </Link>
            </div>
          </div>

          {/* Right Side: Mockup Image */}
          <div className="w-full  flex items-center justify-center order-1 lg:order-2 ">
            <div className="relative w-full sm:w-[65%] md:w-[55%] lg:w-full max-w-[520px] aspect-[16/16]">
              <Image
                src="/images/slides/cover-letter-ai-img-8.png"
                alt="Mobile app mockup"
                fill
                priority
                className="object-contain drop-shadow-[0_25px_60px_rgba(0,0,0,0.35)]"
              />
            </div>
          </div>
        </div>
      </div>

      {/* Robot Decoration */}
      <div className="pointer-events-none absolute bottom-0 left-0 z-10 hidden sm:block">
        <Image
          src="/images/bot.png"
          alt="Decoration"
          width={150}
          height={150}
          className="h-auto w-[100px] lg:w-[150px]"
          priority
        />
      </div>
    </section>
  );
}
