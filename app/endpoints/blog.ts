async function getLatestBlogPosts() {
 const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/blogs?filters[appName][$eq]=cover-letter-ai`,{
  cache: 'no-store',
 });
...
async function getBlogPostsPage(options?: { page?: number; pageSize?: number }) {
  ...
  const baseUrl = `${process.env.NEXT_PUBLIC_API_URL}/blogs?filters[appName][$eq]=cover-letter-ai`;
